...
if ! command -v curl >/dev/null 2>&1; then
  echo "curl wird fuer den Health-Check benoetigt."
  exit 1
fi
...
BACKUP_DIR="${PLAINBASE_BACKUP_DIR:-/var/backups/plainbase}"
...
DB_PATH="${PLAINBASE_DB_PATH:-${DATA_DIR}/plainbase.sqlite}"
API_HOST="${HOST:-127.0.0.1}"
API_PORT="${PLAINBASE_API_PORT:-3001}"

if [ -f "${DB_PATH}" ]; then
  cp "${DB_PATH}" "${BACKUP_DIR}/plainbase-$(date +%Y%m%d%H%M%S).sqlite"
fi
...
chown -R "${APP_USER}:${APP_GROUP}" "${TARGET_DIR}" "${CONFIG_DIR}" "${DATA_DIR}" "${BACKUP_DIR}"
ln -sfn "${TARGET_DIR}" "${CURRENT_LINK}"

systemctl daemon-reload
systemctl restart "${SERVICE_NAME}"

if ! curl --fail --silent --show-error "http://${API_HOST}:${API_PORT}/api/health" >/dev/null; then
  echo "Health-Check fehlgeschlagen. Rolle auf vorheriges Release zurueck."
...
echo "Plainbase wurde auf Version ${VERSION} aktualisiert."
echo "Aktives Release: ${TARGET_DIR}"
